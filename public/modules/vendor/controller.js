...
munchapp.controller("vendorController", ['$http', '$stateParams', '$state', '$scope', '$rootScope', '$localStorage', 'VendorService', 'ngTableParams', '$route', '$location', '$timeout', 'SweetAlert', '$filter', function($http, $stateParams, $state, $scope, $rootScope, $localStorage, VendorService, ngTableParams, $route, $location, $timeout, SweetAlert, $filter) {
	...
	// 	console.log("resonse JJJJJJJJJ",response);
	// 		for(var i=0;i<response.length;i++){
	// 			console.log("aaa",i,response[i].tot_sales)
	// 			arr.push(response[i].tot_sales);
	// 			vendor_name.push(response[i].vendor_details[0].vendor_name
	// 				)
	// 		}
	// 	$scope.weeksale=arr;
	// 	$scope.monthsale=vendor_name;
	// });
	// VendorService.revenueData(inputJSon,function(response){
	// 	$scope.monthrev=response.data.monthrevenue;
	// 	$scope.weekrev=response.data.weekrevenue;
	// });
